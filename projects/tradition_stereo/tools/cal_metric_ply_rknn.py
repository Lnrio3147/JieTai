"""
从PLY点云反投影生成视差图并与GT计算指标 (RKNN版本)

功能：
1. 读取 D:\Desktop\过滤后\原始ply点云_rknn 中的 output_point_cloud.ply
2. 将PLY点云转换为视差图
...
RESULTS_DIR = os.path.join(GT_ROOT, "evaluation_results_ply_rknn")
...
def evaluate_scene(scene_name, ply_folder, gt_folder):
    """
    评估单个场景的视差图
...
def evaluate_all(epe_threshold=10.0):
    """
    评估所有场景
    ...
    """
    print(f"\n{'='*60}")
    print(f"评估 RKNN 过滤后点云: {PLY_FILENAME}")
    print(f"{'='*60}")
    ...
    for scene_name in tqdm(ply_scenes, desc="评估进度"):
        ...
        metrics = evaluate_scene(scene_name, ply_folder, gt_folder)
...
def visualize_results(metrics_df, avg_metrics):
    """
    可视化和保存结果
    """
    # 1. 生成表格
...
总体统计:
    评估场景数: {avg_metrics['Scene Count']}
    EPE: {avg_metrics['EPE']:.4f} px
    D1: {avg_metrics['D1']:.2f} %
    Bad1: {avg_metrics['Bad1']:.2f} %
    Bad2: {avg_metrics['Bad2']:.2f} %
    Bad3: {avg_metrics['Bad3']:.2f} %
    有效像素: {avg_metrics['Valid Pixels']:,}
    总像素: {avg_metrics['Total Pixels']:,}
...
"""
    ...
    # 保存报告
    report_file = os.path.join(RESULTS_DIR, f"rknn_evaluation_{timestamp}.txt")
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(report)

    # 保存CSV
    csv_file = os.path.join(RESULTS_DIR, f"rknn_details_{timestamp}.csv")
...
def main():
    ...
    主函数
    """
    print(f"开始评估RKNN过滤后点云反投影视差图指标...")
    print(f"PLY点云路径: {PLY_ROOT}")
    print(f"GT数据路径: {GT_ROOT}")

    metrics_df, avg_metrics = evaluate_all()

    if metrics_df is not None and avg_metrics is not None:
        visualize_results(metrics_df, avg_metrics)
    else:
        print("错误: 评估失败")
