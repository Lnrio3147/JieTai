...
def evaluate_scene(scene_dir):
    ...
    # 查找预测文件（兼容两种格式）
    candidate_files = glob.glob(os.path.join(scene_dir, "*_disp_cropped.npy"))   # 传统算法
    if candidate_files:
        pred_file = candidate_files[0]  # 使用第一个匹配的传统算法文件
    else:
        # 如果没有找到传统算法文件，尝试IGEV文件
        igev_file = os.path.join(scene_dir, "disp_igev.npy")
        if os.path.exists(igev_file):
            pred_file = igev_file
        else:
            print(f"  警告: 在 {scene_dir} 中未找到预测视差图 (*_disp_cropped.npy 或 disp_igev.npy)")
            return None
