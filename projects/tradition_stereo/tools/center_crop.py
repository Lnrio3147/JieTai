...
def visualize_disparity_with_imshow(disp_path, crop_h, crop_w):
    """
    使用OpenCV的imshow可视化视差图及其中心裁剪版本
    ...
    try:
        ...
        print(f"成功加载视差图，尺寸: {disp.shape[1]}×{disp.shape[0]}")
    ...
    if start_y + crop_h > h or start_x + crop_w > w:
        crop_h = h - start_y
        crop_w = w - start_x
        print(f"调整裁剪尺寸为: {crop_w}×{crop_h}")

    disp_cropped = disp[start_y:start_y + crop_h, start_x:start_x + crop_w]
    print(f"中心裁剪后尺寸: {disp_cropped.shape[1]}×{disp_cropped.shape[0]}")
    ...
    cropped_label = f"Cropped: {crop_w}×{crop_h}"

    # 在图像上添加文本
    cv2.putText(canvas, full_label, (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.putText(canvas, cropped_label, (full_gray.shape[1] + 20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
if __name__ == "__main__":
    ...
    cropped_disp = visualize_disparity_with_imshow(disp_path, crop_h, crop_w)
