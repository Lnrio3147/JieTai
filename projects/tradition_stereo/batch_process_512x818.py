"""
批量处理已裁剪的 512x818 视差图，生成点云和 pointmap
针对 D:\Desktop\scene_demo-imgs_512x832_crop_first 数据
"""
import numpy as np
import cv2
...
def process_single_folder(input_folder, output_folder, image_root=None,
                         min_disparity=5.0, max_disparity=300.0,
                         black_threshold=15):
    """
    处理单个文件夹，生成点云并保存
    输入视差图已经是裁剪后的 512x818，无需再裁剪
    ...
    # 检查必要文件是否存在
    disp_candidates = [
        os.path.join(input_folder, "disp_512x818_crop.npy"),
        os.path.join(input_folder, "disp.npy"),
    ]
    disp_path = next((path for path in disp_candidates if os.path.exists(path)), None)

    image_folder = os.path.join(image_root, folder_name) if image_root else input_folder
    im0_path = os.path.join(image_folder, "im0.png")
    im1_path = os.path.join(image_folder, "im1.png")

    disp_png_candidates = [
        os.path.join(input_folder, "disp_igev_512x818_crop.png"),
        os.path.join(input_folder, "disp_crop.png"),
        os.path.join(input_folder, "vis.png"),
    ]
    disp_igev_path = next((path for path in disp_png_candidates if os.path.exists(path)), None)

    if disp_path is None or not os.path.exists(im0_path):
        print(f"  ❌ 跳过 {folder_name}: 缺少必要文件 (disp.npy/disp_512x818_crop.npy 或 im0.png)")
        return False
    ...
    try:
        ...
        h_disp, w_disp = disparity.shape[:2]
        h_img, w_img = imgL.shape[:2]

        print(f"  视差图尺寸: {h_disp} × {w_disp}")
        print(f"  原始图像尺寸: {h_img} × {w_img}")
        ...
        # 计算动态调整的长宽比裁剪（与原脚本一致）
        roi_width = w_img - 2 * edgeL
        roi_height = h_img - edge
        ...
        print(f"  裁剪图像尺寸: {imgL.shape[0]} × {imgL.shape[1]}")

        # 检查裁剪后尺寸是否匹配
        if imgL.shape[:2] != disparity.shape[:2]:
            print(f"  ⚠️ 尺寸不匹配: 图像 {imgL.shape[:2]} vs 视差 {disparity.shape[:2]}")
            # 调整图像尺寸以匹配视差图
            imgL = cv2.resize(imgL, (w_disp, h_disp))
            print(f"  调整后图像尺寸: {imgL.shape[0]} × {imgL.shape[1]}")

        # 3. 对视差图进行双边滤波，去除噪声同时保持边缘
        ...
        Q_jxp[0, 3] += start_x  # cx' = cx - start_x
        Q_jxp[1, 3] += start_y  # cy' = cy - start_y

        print(f"  Q矩阵调整: start_x={start_x}, start_y={start_y}")

        # 使用Q矩阵和滤波后的视差图进行3D重投影
        ...
        pointmap[mask, :3] = pts3d[mask]
        pointmap[mask, 3:] = imgL_rgb[mask]
...
def main():
    # ==================== 配置参数 ====================
    # 路径配置
    # input_root = r"D:\Desktop\test_lite\FDJYP-0-disparity"
    # image_root = r"D:\Desktop\stereo_project\tradition_stereo\rec_img_set\FDJYP-0-rectified_images"
    # output_root = r"D:\Desktop\test_lite\FDJYP-0-disparity_ply"


    # input_root = r"D:\Desktop\test_lite\FDJYP-3-disparity"
    # image_root = r"D:\Desktop\stereo_project\tradition_stereo\rec_img_set\FDJYP-3-rectified_images"
    # output_root = r"D:\Desktop\test_lite\FDJYP-3-disparity_ply"


    # input_root = r"D:\Desktop\test_lite\lkedu-disparity"
    # image_root = r"D:\Desktop\stereo_project\tradition_stereo\rec_img_set\kedu"
    # output_root = r"D:\Desktop\test_lite\lkedu-disparity_ply"
    ...
    for input_folder in tqdm(input_folders, desc="处理进度"):
        if process_single_folder(input_folder, output_root, image_root=image_root,
