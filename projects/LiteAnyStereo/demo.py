...
def read_color_image(path):
    ...
    if image.ndim != 3 or image.shape[2] < 3:
        raise ValueError(f"Expected an RGB/RGBA image at {path}, got shape {image.shape}")
    return image[..., :3]


def load_stereo_images(args):
    separate_pair = args.left_image is not None or args.right_image is not None
    if separate_pair:
        if args.left_image is None or args.right_image is None:
            raise ValueError("--left and --right must be provided together")
        img0 = read_color_image(args.left_image)
        img1 = read_color_image(args.right_image)
        if img0.shape != img1.shape:
            raise ValueError(f"Left/right image shape mismatch: {img0.shape} vs {img1.shape}")
        return img0, img1

    stereo_image = read_color_image(args.stereo_file)
    height, combined_width = stereo_image.shape[:2]
    if combined_width % 2 != 0:
        stereo_image = stereo_image[:, :-1, :]
        combined_width -= 1
    img0 = stereo_image[:, : combined_width // 2, :]
    img1 = stereo_image[:, combined_width // 2 :, :]
    if img0.shape != img1.shape:
        raise ValueError(f"Split stereo image shape mismatch: {img0.shape} vs {img1.shape}")
    return img0, img1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--left", "--left_image", dest="left_image", default=None, help="rectified left image")
    parser.add_argument("--right", "--right_image", dest="right_image", default=None, help="rectified right image")
    parser.add_argument("--restore_ckpt", default=None, type=str, help="checkpoint path; use 'none' to skip loading")
    parser.add_argument("--out_dir", default="./runs/inference/demo", type=str, help="directory to save results")
    ...
    img0, img1 = load_stereo_images(args)
    if args.scale != 1:
    ...
    if args.get_pc:
        logging.warning(
            "Point-cloud conversion uses the demo camera intrinsics and baseline hard-coded in demo.py; "
            "use --get_pc 0 unless they match the camera that captured these images."
        )
